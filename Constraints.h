...
void rodConstraint(ConstraintObject& co) {
  ...
  Vector3D posDelta =  co.m_obj2->m_position - co.m_obj1->m_position;
  FixedPoint currentLength = posDelta.magnitude();
  debug("Running RodConstraint of length ",DEBUG_CONSTRAINT);
  debug(TO_FLOAT(co.m_rodData.m_length), DEBUG_CONSTRAINT);
  debug(" and current length ",DEBUG_CONSTRAINT);
  debugln(TO_FLOAT(currentLength), DEBUG_CONSTRAINT);
  //co.m_obj1->print();
  //co.m_obj2->print();
  
  posDelta.normalize();
  
  if( currentLength < co.m_rodData.m_length ) {
    debug("Rod too short by ",DEBUG_CONSTRAINT);
    FixedPoint delta = co.m_rodData.m_length - currentLength;
    debugln(TO_FLOAT(delta), DEBUG_CONSTRAINT);
  ...
  else if( currentLength > co.m_rodData.m_length ) {
